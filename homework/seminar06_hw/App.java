...
class Infrastructure {
  ...
  public char[] requestChar(String request) {
  ...
  public void search(String request) {
    ...
    for (int i = 1; i <= db.films.size(); i++) {
      ...
      boolean check = res.contains(request);
      if (check == true) {
        result.add(res + "\n");
  ...
  Db init() {
    db = new Db();
    Cinema c1 = new Cinema(1, "Мстители", 1, 1);
    Cinema c2 = new Cinema(2, "Король Лев", 1, 2);
    Cinema c3 = new Cinema(3, "Гарри Поттер", 3, 4);
